...
type GenSetupFn func(cbdcApp *app.App, genesisState app.GenesisState, customGenesis interface{}) (app.GenesisState, error)
...
func GenStateSetter[T proto.Message](moduleName string) GenSetupFn {
	return func(cbdcApp *app.App, genesisState app.GenesisState, customGenesis interface{}) (app.GenesisState, error) {
		moduleGenesis, ok := customGenesis.(T)
		if !ok {
			return nil, fmt.Errorf("invalid type %T for %s module genesis state", customGenesis, moduleName)
		}

		genesisState[moduleName] = cbdcApp.AppCodec().MustMarshalJSON(moduleGenesis)
...
func CustomizeGenesis(cbdcApp *app.App, customGen CustomGenesisState, genesisState app.GenesisState) (app.GenesisState, error) {
	var err error
	for mod, modGenState := range customGen {
		if fn, found := genesisSetupFunctions[mod]; found {
			genesisState, err = fn(cbdcApp, genesisState, modGenState)
			if err != nil {
...
// createcbdcApp creates an cbdc app
func CreatecbdcApp(chainID string, customBaseAppOptions ...func(*baseapp.BaseApp)) *app.App {
	// Create cbdc app
	loadLatest := true
