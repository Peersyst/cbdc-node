...
func New(opts ...cbdccommon.ConfigOption) *IntegrationNetwork {
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	cbdcApp := cbdccommon.CreatecbdcApp(n.cfg.ChainID, n.cfg.CustomBaseAppOpts...)
	...
	slashingParams, err := getValidatorsSlashingGen(validators, cbdcApp.StakingKeeper)
	if err != nil {
		return err
	}
	...
	genesisState, err = customizeGenesis(cbdcApp, n.cfg.CustomGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	consensusParams := cbdccommon.DefaultConsensusParams
	...
	header := cmtproto.Header{
		ChainID:            n.cfg.ChainID,
		Height:             cbdcApp.LastBlockHeight() + 1,
		AppHash:            cbdcApp.LastCommitID().Hash,
		Time:               now,
	...
	if _, err := cbdcApp.FinalizeBlock(req); err != nil {
		return err
	}

	// TODO - this might not be the best way to initilize the context
	n.ctx = cbdcApp.BaseApp.NewContextLegacy(false, header)

	// Commit genesis changes
	if _, err := cbdcApp.Commit(); err != nil {
		return err
	}
