...
func SubmitProposal(tf factory.BaseTxFactory, network cbdcintegration.Network, proposerPriv cryptotypes.PrivKey, title string, msgs ...sdk.Msg) (uint64, error) {
...
// return the proposal ID.
func SubmitLegacyProposal(tf factory.BaseTxFactory, network cbdcintegration.Network, proposerPriv cryptotypes.PrivKey, proposal govv1beta1.Content) (uint64, error) {
...
func ApproveProposal(tf factory.BaseTxFactory, network cbdcintegration.Network, proposerPriv cryptotypes.PrivKey, proposalID uint64) error {
	// Vote on proposal
...
func SubmitAndAwaitProposalResolution(tf factory.BaseTxFactory, network cbdcintegration.Network, keys []keyring.Key, title string, msgs ...sdk.Msg) (*govv1.Proposal, error) {
...
func retrieveResolvedProposal(network cbdcintegration.Network, proposalID uint64) (*govv1.Proposal, error) {
...
func submitProposal(tf factory.BaseTxFactory, network cbdcintegration.Network, proposerPriv cryptotypes.PrivKey, txArgs factory.CosmosTxArgs) (uint64, error) {
...
func waitVotingPeriod(n cbdcintegration.Network) error {
	gq := n.GetGovClient()
...
func checkProposalStatus(n cbdcintegration.Network, proposalID uint64, expStatus govv1.ProposalStatus) error {
	gq := n.GetGovClient()
